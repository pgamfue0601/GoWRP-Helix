...
timer.Simple(30, function()
	...
	ix.chat.Register("scream", {
		...
		description = "Grita con todas tus fuerzas.",
		font = "ixScreamFont",
		OnChatAdd = function(self, speaker, text)
			...
			chat.AddText(color, name, color, " grita a pleno pulmón ¡¡\"" .. text .. "\"!!")
		end
	})
	...
	ix.chat.Register("meclose", {
		...
		CanHear = ix.config.Get("chatRange", 280) * 0.25,
		prefix = {"/MeClose", "/MeC"},
		description = "Realiza una acción de distancia corta, algo no muy notable.",
		indicator = "chatPerforming",
		deadCanChat = true,
		GetColor = function(self, speaker, text)
	...
	ix.chat.Register("mefar", {
		...
		CanHear = ix.config.Get("chatRange", 280) * 2,
		prefix = {"/MeFar", "/MeF"},
		description = "Realiza una acción que se pueda destacar desde largas distancias.",
		indicator = "chatPerforming",
		deadCanChat = true,
		GetColor = function(self, speaker, text)
			local color = ix.chat.classes.me:GetColor(speaker, text)
	...
	ix.chat.Register("mefarfar", {
		...
		description = "Realiza una acción que se pueda destacar desde distancias enormes.",
		indicator = "chatPerforming",
		deadCanChat = true,
		GetColor = function(self, speaker, text)
			local color = ix.chat.classes.me:GetColor(speaker, text)
